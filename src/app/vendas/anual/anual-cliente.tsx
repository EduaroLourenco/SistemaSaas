...
import { derivar, somar, type MesAnual } from "@/mock/anual";
import type { DadosAnual } from "@/lib/dados/vendas";

/** O escopo é o id de um canal, ou "todos". Sai do banco, não de um union fixo. */
type EscopoAnual = string;
import {
  Bar,
  CartesianGrid,
  ComposedChart,
...
function montarEscopos(canais: DadosAnual["canais"]) {
  return [
    { value: "todos" as EscopoAnual, label: "Todos os canais" },
    ...canais.map((c) => ({ value: c.id as EscopoAnual, label: c.nome })),
  ];
}
...
export default function VendasAnual({ dados }: { dados: DadosAnual }) {
  const {
    ano: ANO,
  ...
  const ESCOPOS = React.useMemo(() => montarEscopos(CANAIS_ANUAL), [CANAIS_ANUAL]);
  const nomeEscopo = React.useCallback(
    (e: EscopoAnual) => ESCOPOS.find((o) => o.value === e)?.label ?? "Todos os canais",
    [ESCOPOS]
  );
  const ANUAL_POR_CANAL = ANUAL_SERIES;
...
function DetalheMes({
  ...
  anterior?: MesAnual;
  ano: number;
  canais: DadosAnual["canais"];
  series: DadosAnual["series"];
  nomeEscopo: (e: EscopoAnual) => string;
  onClose: () => void;
}) {
  const d = derivar(m);
  const f = faixaMeta(d.pctMeta);
