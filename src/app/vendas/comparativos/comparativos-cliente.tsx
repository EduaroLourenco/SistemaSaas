...
export default function VendasComparativos({ dados }: { dados: DadosComparativos }) {
  const { ano: ANO, canais, serieDiaria } = dados;

  const ESCOPOS_COMP = React.useMemo(
    () => [
      { value: "todos" as EscopoComp, label: "Todos os canais" },
      ...canais.map((c) => ({ value: c.id as EscopoComp, label: c.nome })),
    ],
    [canais]
  );
  const nomeEscopo = React.useCallback(
    (e: EscopoComp) => ESCOPOS_COMP.find((o) => o.value === e)?.label ?? "Todos os canais",
    [ESCOPOS_COMP]
  );
  const agregar = React.useCallback(
    (indices: number[], escopo: EscopoComp) =>
      agregarSerie(serieDiaria[escopo] ?? [], indices),
    [serieDiaria]
  );
