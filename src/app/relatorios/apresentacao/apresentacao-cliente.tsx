...
import type { FormatoDeck } from "@/mock/relatorios";
...
import { montarRoteiro, type Slide } from "@/lib/roteiro";
import type { DadosApresentacao } from "@/lib/dados/apresentacao";


/** Recortes prontos. "Personalizada" abre o calendário. */
const FORMATOS: { value: FormatoDeck; label: string; descricao: string }[] = [
  { value: "diaria", label: "Diária", descricao: "O último dia com movimento, contra o dia anterior." },
  { value: "semanal", label: "Semanal", descricao: "Os últimos 7 dias, contra os 7 anteriores." },
  { value: "personalizada", label: "Personalizada", descricao: "Escolha o intervalo — a comparação é com o período de mesmo tamanho logo antes." },
];
...
/** Recuo em dias a partir de uma data ISO. */
...
export default function Apresentacao({ dados }: { dados: DadosApresentacao }) {
  const [formato, setFormato] = React.useState<FormatoDeck>("semanal");
  const [canal, setCanal] = React.useState("");
  const [emTelaCheia, setEmTelaCheia] = React.useState(false);

  /*
   * O intervalo personalizado nasce igual ao semanal, para o calendário
   * abrir num recorte válido em vez de vazio.
   */
  const [de, setDe] = React.useState(() =>
    dados.ultimaData ? recuar(dados.ultimaData, 6) : ""
  );
  const [ate, setAte] = React.useState(dados.ultimaData);

  const intervalo = React.useMemo(() => {
    if (!dados.ultimaData) return { de: "", ate: "" };
    if (formato === "diaria") return { de: dados.ultimaData, ate: dados.ultimaData };
    if (formato === "semanal")
      return { de: recuar(dados.ultimaData, 6), ate: dados.ultimaData };
    // Personalizada: inverte se vier ao contrário, em vez de devolver vazio.
    return de <= ate ? { de, ate } : { de: ate, ate: de };
  }, [formato, de, ate, dados.ultimaData]);
  ...
  const SLIDES: Slide[] = React.useMemo(
    ...
    [dados, intervalo, canal]
  );

  const [selecionados, setSelecionados] = React.useState<string[]>([]);

  // Slide novo entra marcado; o que sumiu do roteiro sai da seleção.
  React.useEffect(() => {
    setSelecionados((atual) => {
      const ids = SLIDES.map((s) => s.id);
      const mantidos = atual.filter((id) => ids.includes(id));
      return mantidos.length ? mantidos : ids;
    });
  }, [SLIDES]);
