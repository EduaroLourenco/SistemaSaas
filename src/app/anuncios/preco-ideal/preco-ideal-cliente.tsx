...
import type { DadosPrecoIdeal } from "@/lib/dados/preco-ideal";
...
export default function PrecoIdeal({ dados }: { dados: DadosPrecoIdeal }) {
  const {
  ...
  // O cruzamento vem pronto do servidor: cruzar catálogo com Fórmula base
  // no navegador significaria mandar 24 mil linhas de preço para o cliente.
  const cruzar = React.useCallback(
    (rel: RelatorioPrecoIdeal): LinhaCruzada[] => cruzamentos[rel.id] ?? [],
    [cruzamentos]
  );
